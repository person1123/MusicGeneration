...
def train(filepath, chain=None, chain_depth = 2, bin_size = 0.00001):
    global samplerate
...
def read(filename):
    with open(filename) as infile:
        chain = json.read(infile)
        return chain
    return None
        
def train_and_write(filepath, chainpath, chain_depth = 2, bin_size = 0.00001):
    chain = read(chainpath)
    chain = train(filepath, chain, chain_depth, bin_size)
    write(chain, chainpath)

def read_and_gen(chainpath,filepath):
    chain = read(chainpath)
    gen(chain,filepath)
"""
...
def gen(chain,filepath):
    ...
    for i in range(new_data.size):
        ...
        for k in tc.keys():
            if k is not "sum" and k is not "samplerate":
...
if __name__=="__MAIN__":
    ...
    parser_train.add_argument("-chain")
    parser_train.add_argument("-chain_depth")
    parser_train.add_argument("-bin_size")
    ...
    parser_write.add_argument("filepath")
