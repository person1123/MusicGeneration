...
batch_size = 128
np_epoch = 10
output_n_frames = f.samplerate*15
...
for i in range(max_i):
	...
	sys.stdout.flush()
	frames[i+seq_len] = model.predict_classes(np.array([data[i:i+seq_len]]), batch_size=1, verbose=0)

sys.stdout.write("Status Saving file...          \r")
frames = frames/(buckets/2.0) - 1.0
print frames

f2 = audiolab.Sndfile('test4b.wav', 'w', audiolab.Format('wav'), 1, sampling_rate)
